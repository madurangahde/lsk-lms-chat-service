...
import { mergeMessages, resolveErrorMessage, sortConversations, upsertConversation } from '../utils/chat.js';
...
export default function AdminChatPage() {
  ...
  const loadConversations = async (searchValue = search, onlyUnreadValue = onlyUnread) => {
  ...
  const loadMessages = async (conversationId) => {
    ...
    try {
      const response = await http.get(`/admin/conversations/${conversationId}/messages`, {
        params: { limit: 100 }
      });
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (!mounted) return;
        setError(resolveErrorMessage(err, 'Failed to load admin conversations'));
  ...
  useEffect(() => {
    ...
    const onConnectError = (err) => setError(err?.message || 'Socket connection failed');
    const onConversationUpdated = (conversation) => {
      setConversations((prev) => upsertConversation(prev, conversation));
      setSelectedConversation((prev) => {
        if (!prev) return prev;
        return prev.id === conversation.id ? { ...prev, ...conversation } : prev;
      });
    };
  ...
  useEffect(() => {
    ...
    socket.emit('conversation:subscribe', { conversationId: currentId }, () => {});
    return () => {
      socket.emit('conversation:unsubscribe', { conversationId: currentId }, () => {});
    };
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      const response = await new Promise((resolve) => {
        socket.emit('message:admin:send', { conversationId: selectedConversation.id, text }, resolve);
      });
      ...
      setConversations((prev) => upsertConversation(prev, response.data.conversation));
      setSelectedConversation((prev) => ({ ...(prev || {}), ...response.data.conversation }));
      return true;
    } catch (err) {
  ...
  const handleBroadcast = async (text) => {
    ...
    try {
      ...
      await loadConversations(search, onlyUnread);
      setError(`Broadcast sent successfully to ${response.data.deliveredUsers} users.`);
      return true;
    } catch (err) {
    ...
    <AppShell
      topbar={
        <TopBar
          user={user}
          onLogout={logout}
          rightSlot={<div className={`status-chip ${socketState}`}>{socketState}</div>}
        />
      }
      sidebar={
        <div className="sidebar-stack">
          ...
          <label className="checkbox-row">
            <input type="checkbox" checked={onlyUnread} onChange={handleUnreadToggle} />
      ...
      content={
        <div className="chat-panel chat-panel--admin">
          ...
          {error && <div className={error.startsWith('Broadcast sent') ? 'success-banner' : 'error-banner'}>{error}</div>}
          {selectedConversation ? (
            <>
              <div className="panel-header panel-header--stacked">
                <div>
                  <div className="panel-title">{selectedConversation.userName}</div>
                  <div className="muted small">
                    {selectedConversation.userEmail || selectedConversation.userId} • unread for admin:{' '}
              ...
              {loadingMessages ? (
                ...
                <MessageList items={messages} currentUser={user} emptyLabel="No messages in this conversation yet." />
              )}
              <MessageComposer onSend={handleSend} sending={sending} placeholder="Reply to this user..." />
            </>
          ) : (
            <div className="panel-empty">Select a conversation to start replying.</div>
          )}
        </div>
      }
    />
